...
{
  const rclcpp::Time time_a(pose_a.header.stamp);
  const rclcpp::Time time_b(pose_b.header.stamp);
...
void getNearestTimeStampPose(
  ...
  geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr & output_old_pose_cov_msg_ptr,
  geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr & output_new_pose_cov_msg_ptr)
{
  for (const auto & pose_cov_msg_ptr : pose_cov_msg_ptr_array) {
    output_new_pose_cov_msg_ptr = pose_cov_msg_ptr;
    if (rclcpp::Time(output_new_pose_cov_msg_ptr->header.stamp) > time_stamp) {
      // TODO refactor
      if (rclcpp::Time(output_old_pose_cov_msg_ptr->header.stamp).seconds() == 0.0) {
  ...
  const rclcpp::Time & time_stamp)
{
  const rclcpp::Time time_a(pose_a.header.stamp);
  const rclcpp::Time time_b(pose_b.header.stamp);
  if (time_a.seconds() == 0.0 || time_b.seconds() == 0.0 || time_stamp.seconds() == 0.0) {
    return geometry_msgs::msg::PoseStamped();
  }

  const auto twist = calcTwist(pose_a, pose_b);
  const double dt = (time_stamp - time_a).seconds();
