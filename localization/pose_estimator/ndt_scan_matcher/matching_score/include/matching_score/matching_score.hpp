...
public:
  ...
  void setInputTarget(const std::shared_ptr<pcl::PointCloud<PointType> const> & pointcloud_ptr);
  void setSearchMethodTarget(const std::shared_ptr<pcl::search::KdTree<PointType>> & tree_ptr);
  double calcMatchingScore(
    const std::shared_ptr<pcl::PointCloud<PointType> const> & pointcloud_ptr);
...
private:
  ...
  std::shared_ptr<pcl::search::KdTree<PointType>> tree_ptr_;
