...
public:
  ...
  void setInputTarget(const std::shared_ptr<pcl::PointCloud<PointTarget>> & map_ptr) override;
  void setInputSource(const std::shared_ptr<pcl::PointCloud<PointSource>> & scan_ptr) override;
  ...
  std::shared_ptr<const pcl::PointCloud<PointTarget>> getInputTarget() const override;
  std::shared_ptr<const pcl::PointCloud<PointSource>> getInputSource() const override;
  ...
  std::shared_ptr<pcl::search::KdTree<PointTarget>> getSearchMethodTarget() const override;
...
private:
  std::shared_ptr<ndt_omp::NormalDistributionsTransform<PointSource, PointTarget>> ndt_ptr_;
};
