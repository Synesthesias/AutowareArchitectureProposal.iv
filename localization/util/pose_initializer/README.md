...
### 主な特長 (Key Features)
- **高さ補正付きシーディング**: `pointcloud_map` のキャッシュを半径 1 m 以内でサンプリングし、TF 変換を適用して入力姿勢を地図面へスナップしてから publish します。
- **手動初期姿勢の整列**: `initialpose` で与えられた姿勢に緩めの平面共分散を設定し、`ndt_align_srv` で位置合わせしたあとにタイトな共分散へ置き換えて配信します。
- **GNSS ワンショット初期化**: `use_first_gnss_topic` が true のとき `gnss_pose_cov` を 1 回だけ利用し、整列結果を publish した直後に購読を停止して不要な再初期化を防ぎます。
...
- **アライン監視**: `ndt_align_srv` が 1 s 待っても現れない場合や応答が 3 s 以内に返らない場合は WARN を出し、高さ補正のみの姿勢をフォールバックとして publish します。
...
| `gnss_pose_cov`  | `geometry_msgs/msg/PoseWithCovarianceStamped` | `SensorDataQoS()`                               | 任意の GNSS シード。`use_first_gnss_topic` が true の場合は 1 回 publish 後に購読解除します。 |
...
| F-003   | GNSS ブートストラップは 1 回だけ publish し、その後サブスクリプションを停止する。                       | UT_PI_003 | 最初の `gnss_pose_cov` のみ `initialpose3d` を生成し、2 件目以降は無視されること。                                       |
| F-004   | `pose_initializer_srv` がトピック経路と同じ処理を行い、成功時は必ず publish する。                      | UT_PI_004 | サービス応答の姿勢が `initialpose3d` にも出力されること。                                                                |
...
| F-006   | `ndt_align_srv` が不在/タイムアウトでも高さ補正のみでフォールバックし、警告を出し続けるウォッチドッグ。 | IT_PI_001 | アラインサービス不在時に WARN が 1 Hz で出力され、高さ補正のみの姿勢が publish されてノードが落ちない。                  |
...
| UT_PI_002 | 手動 `initialpose` が NDT 整列と publish をトリガすることを確認。 | テスト executor 上でコンポーネントを動かし、既知の姿勢を返す `ndt_align_srv` モックと `initialpose3d` 購読者を用意する。                         | `initialpose` を 1 件 publish する。                            | モックサーバが 1 リクエストを受け、購読者は整列後の姿勢と `[1,1,0.01,0.01,0.01,0.2]` の共分散を受信し、ヘッダ stamp は入力からコピーされる。                      |
| UT_PI_003 | GNSS 購読が 1 回で停止することを確認。                            | `use_first_gnss_topic:=true` で起動し、整列サービスをモックする。`initialpose3d` を購読しつつ購読者数を監視できるようにする。                    | `gnss_pose_cov` に 2 件のメッセージを publish。                 | 1 件目のみ `initialpose3d` が出力され、2 件目以降は publish されない。購読もリセットされる (ログや購読者数で検証)。                                               |
...
| UT_PI_005 | map frame 変更後も TF が適用されることを確認。                    | `map_aligned` frame の `pointcloud_map` を publish し、`map_aligned`↔`base_link` の TF を放送。                                                  | 地図を受け取った後、`base_link` frame で `initialpose` を送る。 | 高さ補正に `map_aligned` が使われ、TF lookup が成功し、`initialpose3d` の位置が変換後の高さを示す。                                                               |
| IT_PI_001 | 整列サービスが無い場合のフォールバック挙動を確認。                | `pose_initializer.launch.py` を `ndt_align_srv` 未起動のまま `launch_testing_ros` から立ち上げ、RViz 相当の `initialpose` と地図点群を供給する。 | `ndt_align_srv` が無い状態で `initialpose` を publish。         | ログに "NDT align service is unavailable" が出力され、高さ補正のみの姿勢が publish される。ノードはクラッシュせず、後からサービスを起動すれば再度整列を試行する。 |
...
3. 入力を与えます。
   - `pointcloud_map` をラッチ付きで publish し、map frame と入力姿勢フレーム間の TF を用意する。
   - RViz の「2D Pose Estimate」や `/sensing/gnss/pose_with_covariance` への publish で初期化を行う。
